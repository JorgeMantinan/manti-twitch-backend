...
app.get("/auth/twitch/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(
      `https://jorgemantinan.github.io/manti-twitch/?token=${userToken}`
    );
...
app.post("/api/subs", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const baseUrl =
      `https://api.twitch.tv/helix/subscriptions?broadcaster_id=${twitchId}&first=100`;
...
app.get('/api/chatters', verifyToken, async (req, res) => {
  try {
    ...
    const chattersRes = await axios.get(
      "https://api.twitch.tv/helix/chat/chatters",
      {
        params: {
          broadcaster_id: twitchId,
          moderator_id: twitchId
        },
        headers: {
    ...
    res.json({
      total: listaNombres.length,
      chatters: listaNombres
    });

  } catch (e) {

    console.error("Chatters error:", e.response?.data || e.message);

    res.status(500).json({
      error: "No se pudo obtener chatters"
    });
...
app.post('/api/raffle/start', verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (!userRes.data.data.length) {
          return res.status(400).json({ error: "No se pudo obtener el usuario de Twitch" });
        }
...
client.on('message', (channel, tags, message, self) => {
  ...
  if (globalChatLogs.length > 10000) {
    globalChatLogs.shift();
  }

  if (!message.toLowerCase().includes(raffleState.keyword.toLowerCase())) {
    return;
  }
  ...
  if (userData) {
    if (userData.isSub) {
      tickets *= raffleState.subMult;
    }
    if (userData.giftsSent > 0) {
      tickets += userData.giftsSent * raffleState.giftMult;
    }
  }
    ...
    if (random < p.tickets) {
      return res.json({
        ...
        stats: {
          ...
          probability: ((p.tickets / totalTickets) * 100).toFixed(2) + "%"
        }
      });
    }
    random -= p.tickets;
