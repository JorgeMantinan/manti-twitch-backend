...
io.on("connection", (socket) => {
  ...
  */
  ...
  socket.on("bingo:draw", () => {
    ...
    if (!room.started) {
      console.log("⚠️ Bingo not started");
      return;
    }
...
function getBingoRoom(streamer) {
  if (!bingoRooms[streamer]) createBingoRoom(streamer);
...
async function getTwitchIdByNick(nick, accessToken) {
  const response = await fetch(
    `https://api.twitch.tv/helix/users?login=${nick}`,
    {
      headers: {
  ...
  try {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        allSubs = await fetchAllTwitchData(baseUrl, newTokens.access_token);
      } else {
        throw err;
      }
    }
  ...
  try {
    ...
    allSubs.forEach((sub) => {
      ...
      if (
        sub.is_gift &&
        gifter &&
        subDate >= raffleState.startDate &&
        subDate <= raffleState.endDate
      ) {
        const gifterData = raffleState.cachedSubs.get(gifter) || {
          isSub: false,
          giftsSent: 0,
        };
    ...
    console.error(
      "Error in Raffle Start:",
      error.response?.data || error.message,
    );
    res
      .status(500)
      .json({ error: "No se pudo iniciar el sorteo o unir el bot al chat." });
  }
});
...
app.post("/api/raffle/pick-winner", (req, res) => {
  ...
  if (participants.length === 0) {
    return res
      .status(400)
      .json({ error: "No hay participantes en el sorteo." });
  }
